...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        ...
        for j in range(K):
            weights[j] = query[i].dot(key[j]) / torch.sqrt(torch.tensor(M).float())
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for k in range(K):
            weights = torch.zeros(K)
            for i in range(K):
                weights[i] = query[n, k].dot(key[n, i]) / torch.sqrt(torch.tensor(M).float())
            weights = torch.softmax(weights, dim=-1)
            for i in range(K):
                out[n, k] += weights[i] * value[n, i]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights = torch.bmm(query, key.transpose(1, 2)) / torch.sqrt(torch.tensor(M).float())
    if mask is not None:
        ##########################################################################
        ...
        # You can check the masked_fill function in PyTorch.                     #
        ##########################################################################
        # Replace "pass" statement with your code
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.q.weight, -math.sqrt(6 / (self.q.in_features + self.q.out_features)), math.sqrt(6 / (self.q.in_features + self.q.out_features)))
        self.k = nn.Linear(dim_in, dim_q)
        nn.init.uniform_(self.k.weight, -math.sqrt(6 / (self.k.in_features + self.k.out_features)), math.sqrt(6 / (self.k.in_features + self.k.out_features)))
        self.v = nn.Linear(dim_in, dim_v)  
        nn.init.uniform_(self.v.weight, -math.sqrt(6 / (self.v.in_features + self.v.out_features)), math.sqrt(6 / (self.v.in_features + self.v.out_features)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        nn.init.uniform_(self.linear.weight, -math.sqrt(6 / (self.linear.in_features + self.linear.out_features)), math.sqrt(6 / (self.linear.in_features + self.linear.out_features)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        y = self.linear(torch.cat(heads, dim=-1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        nn.init.uniform_(self.linear1.weight, -math.sqrt(6 / (self.linear1.in_features + self.linear1.out_features)), math.sqrt(6 / (self.linear1.in_features + self.linear1.out_features)))
        self.linear2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(self.linear2.weight, -math.sqrt(6 / (self.linear2.in_features + self.linear2.out_features)), math.sqrt(6 / (self.linear2.in_features + self.linear2.out_features)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multi_head_attention = MultiHeadAttention(num_heads, emb_dim, emb_dim//num_heads)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.arange(0, K, dtype=torch.float32) / K

    # Reshape the tensor to (1, K, 1)
    y = y.view(1, K, 1)

    # Replicate the tensor M times along the last dimension
    y = y.repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
